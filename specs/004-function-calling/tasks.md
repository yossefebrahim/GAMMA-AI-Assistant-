...
- [X] T001 Add `battery_plus: ^7.0.0` and `android_intent_plus: ^6.0.0` to `pubspec.yaml`
      (research R4 pins, comment each with its seam + confinement note, house style);
      `flutter pub get` resolves clean.
- [X] T002 [P] Android manifest prep in `android/app/src/main/AndroidManifest.xml`: add
...
- [X] T003 [P] Extend `tool/check_plugin_seam.sh`: `battery_plus` and `android_intent_plus`
...
- [X] T004 [P] `lib/domain/entities/generation_event.dart`: sealed `GenerationEvent` —
...
- [X] T005 [P] `lib/domain/entities/tool_spec.dart`: `ToolSpec` (name/description/parameters/
...
- [X] T006 [P] `lib/core/tools/schema_validator.dart`: the R3 subset validator (object root,
...
- [X] T007 Unit tests `test/unit/core/schema_validator_test.dart` (after T006): valid args (empty,
...
- [X] T008 [P] `lib/core/tools/tool_registry.dart`: const four-`ToolSpec` registry with the
...
- [X] T009 Registry sanity tests `test/unit/core/tool_registry_test.dart` (after T006+T008): unique
...
- [X] T010 `lib/domain/services/tool_dispatcher.dart`: `dispatch(name, args) → ToolOutcome` —
...
- [X] T011 Dispatcher unit tests `test/unit/domain/tool_dispatcher_test.dart` with
...
- [X] T012 `lib/data/db/tables.dart` + `lib/data/db/app_database.dart`: add nullable
...
- [X] T013 Migration test `test/data/migration_v3_to_v4_test.dart` (house pattern): seed a real
...
- [X] T014 `lib/domain/entities/message.dart`: `MessageRole.tool`, tool fields, the all-null /
      non-null invariant helpers (data-model §1).
- [X] T015 Repository: `appendToolInvocation` / `finalizeToolInvocation` (terminal-states-only,
...
- [X] T016 Repository tests `test/data/repository_tool_rows_test.dart` (in-memory drift):
...
- [X] T017 `lib/domain/services/gemma_service.dart`: `generate` returns
...
- [X] T018 [P] `lib/infrastructure/gemma/leak_filter.dart`: pure `LeakFilter` — withhold
...
- [X] T019 LeakFilter unit tests `test/unit/infrastructure/leak_filter_test.dart` (after T018)
...
- [X] T020 `lib/infrastructure/gemma/flutter_gemma_service.dart`: thread the structural coupling
...
- [X] T021 Extend `FakeGemmaService` in `test/helpers/fake_gemma_service.dart`: scriptable
...
- [X] T022 Mechanical migration of existing tests/fakes broken by `Stream<String>` →
      ...
      the test layer.
- [X] T023 `lib/features/chat/chat_controller.dart`: consume `GenerationEvent`s for the
...
- [X] T024 `lib/features/chat/context_assembler.dart` (after T014): tool turns in `ChatTurn` replay
...
- [X] T025 [P] [US1] `lib/infrastructure/tools/device_info_tool_service.dart`:
      ...
      fake included.
- [X] T026 [P] [US1] StatFs `MethodChannel` (~10 lines Kotlin) in
      `android/app/src/main/kotlin/.../MainActivity.kt` returning free/total storage bytes
      (R4 — rejected disk_space_plus).
- [X] T027 [P] [US1] `lib/core/model_catalog.dart`: `supportsFunctionCalling = true` composed
      ...
      on the flag.
- [X] T028 [P] [US1] `lib/features/chat/tool_handler_providers.dart`: registry + handler map +
...
- [X] T029 [US1] The R6 tool-use system instruction (lowercase, ~40 tokens) as a const in
      `lib/core/tools/tool_registry.dart`, threaded by the seam when `functionCalling` on
      (exact wording tuned in T049's device pass).
- [X] T030 [US1] Controller tool branch in `lib/features/chat/chat_controller.dart` — the
...
- [X] T031 [US1] Controller loop tests `test/unit/features/chat_controller_tool_test.dart`
...
- [X] T032 [P] [US1] `lib/features/chat/widgets/tool_chip.dart`: design-system §8 — mono
      ...
      non-interactive.
- [X] T033 [US1] Render `role == tool` rows as `ToolChip` in the chat list (message_bubble or
      list builder switch) — chips render regardless of current capabilities (FR-010).
- [X] T034 [US1] Widget tests `test/widget/tool_chip_test.dart` (after T032+T033): all four states render
...
- [X] T036 [P] [US2] set_theme handler in `lib/features/chat/tool_handler_providers.dart`
...
- [X] T037 [US2] Widget test `test/widget/theme_tool_test.dart`: scripted set_theme turn flips
...
- [X] T039 [P] [US3] Capability-gating tests `test/unit/features/tool_gating_test.dart`:
...
- [X] T040 [P] [US3] Widget regression `test/widget/tool_gating_regression_test.dart`:
...
- [X] T042 [P] [US4] `lib/infrastructure/tools/timer_intent_service.dart`: android_intent_plus
...
- [X] T043 [US4] Tests in `test/unit/infrastructure/timer_intent_service_test.dart`: handler
...
- [X] T045 [P] [US5] `lib/infrastructure/tools/clipboard_tool_service.dart`: Flutter
      ...
      the bounded text).
- [X] T046 [US5] Tests in `test/unit/infrastructure/clipboard_tool_service_test.dart`: handler
...
- [X] T048 [P] [US6] Replay mapping in the seam
...
- [X] T050 [P] [US7] Forced-failure controller tests
...
- [X] T051 [P] [US7] Stale-row sweep wiring at conversation open/startup (T016's repo sweep
...
- [X] T053 [P] Static gates: `flutter analyze` zero issues, full `flutter test` green,
      `tool/check_plugin_seam.sh` (battery_plus/android_intent_plus confinement),
      `tool/check_network_seam.sh` (tools add ZERO egress).
- [X] T054 [P] Spike cleanup: remove `integration_test/spike_function_calling_test.dart` and
