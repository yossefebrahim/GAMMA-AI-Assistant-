...
void main() {
  ...
  Message imageMessage({String content = 'look at this'}) => Message(
    ...
    image: const ImageAttachment(
      path: '/tmp/fake-bubble.jpg',
      mimeType: 'image/jpeg',
    ),
  );

  testWidgets(
    'a user message with an image renders the image in place (FR-012)',
    (tester) async {
  ...
  testWidgets(
    'an image-only message (empty text) still renders the image (FR-004/FR-012)',
    (tester) async {
