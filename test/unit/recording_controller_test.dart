...
void main() {
  ...
  test('happy path: mic tap (granted) → recording → stop → previewing with the clip', () async {
    ...
    recorder.nextStop =
        RecordedAudio(path: clipFile.path, mimeType: 'audio/wav', durationMs: 1500);

    await controller.onMicTap();
    expect(container.read(recordingControllerProvider).phase, RecordingPhase.recording);
  ...
  test('denied → request → granted starts recording (first-use request, FR-010)', () async {
    permission
  ...
  test('cap auto-stop keeps the clip with the limit note (FR-002, spec Q1)', () {
    fakeAsync((async) {
      final container = ProviderContainer(overrides: overrides());
      final controller = container.read(recordingControllerProvider.notifier);
      recorder.nextStop = RecordedAudio(
          path: '${tempDir.path}/cap.wav', mimeType: 'audio/wav', durationMs: 30000);
      ...
      expect(container.read(recordingControllerProvider).elapsed,
          const Duration(seconds: 3));

      async.elapse(AudioConstants.maxClipDuration - const Duration(seconds: 3));
      async.flushMicrotasks();

      final state = container.read(recordingControllerProvider);
      expect(state.phase, RecordingPhase.previewing, reason: 'cap keeps the clip');
  ...
  test('a stop under the 500 ms minimum discards the clip with the note (FR-002)', () async {
    final container = makeRecordingContainer();
    final controller = container.read(recordingControllerProvider.notifier);
    final clipFile = tempClip('short.wav');
    recorder.nextStop =
        RecordedAudio(path: clipFile.path, mimeType: 'audio/wav', durationMs: 300);
    ...
    expect(clipFile.existsSync(), isFalse, reason: 'too-short temp is deleted');
  });

  test('remove deletes the temp file, stops the preview, and returns to idle', () async {
    final container = makeRecordingContainer();
    final controller = container.read(recordingControllerProvider.notifier);
    final clipFile = tempClip('rm.wav');
    recorder.nextStop =
        RecordedAudio(path: clipFile.path, mimeType: 'audio/wav', durationMs: 2000);
    await controller.onMicTap();
    await controller.stopRecording();
    ...
    expect(container.read(recordingControllerProvider).phase, RecordingPhase.idle);
    expect(clipFile.existsSync(), isFalse, reason: 'nothing persists from a cancelled compose');
    expect(player.stopCalls, greaterThanOrEqualTo(1), reason: 'preview stopped on remove');
  });
  ...
  test('re-record replaces the previous clip (old temp deleted)', () async {
    ...
    recorder.nextStop =
        RecordedAudio(path: first.path, mimeType: 'audio/wav', durationMs: 1000);
    await controller.onMicTap();
    await controller.stopRecording();
    ...
    recorder.nextStop =
        RecordedAudio(path: second.path, mimeType: 'audio/wav', durationMs: 1200);
    await controller.onMicTap(); // re-record from previewing
    expect(container.read(recordingControllerProvider).phase, RecordingPhase.recording);
    expect(first.existsSync(), isFalse, reason: 'old temp replaced');
    expect(player.stopCalls, greaterThanOrEqualTo(1), reason: 'preview stopped on replace');

    await controller.stopRecording();
    expect(container.read(recordingControllerProvider).clip!.path, second.path);
  });

  test('preview play/stop delegates to the player and tracks its state', () async {
    final container = makeRecordingContainer();
    final controller = container.read(recordingControllerProvider.notifier);
    ...
    expect(container.read(recordingControllerProvider).isPreviewPlaying, isTrue);

    await controller.stopPreview();
    await Future<void>.delayed(Duration.zero);
    expect(container.read(recordingControllerProvider).isPreviewPlaying, isFalse);
    expect(player.stopCalls, 1);
  });

  test('clearAfterSend stops the preview, drops the temp, and returns to idle', () async {
    final container = makeRecordingContainer();
    final controller = container.read(recordingControllerProvider.notifier);
    ...
    expect(clipFile.existsSync(), isFalse, reason: 'composer temp dropped after persist');
    expect(player.stopCalls, greaterThanOrEqualTo(1), reason: 'preview stopped on send');
  });

  test('a recorder-unavailable start surfaces the inline error and stays idle (FR-021)',
      () async {
    final container = makeRecordingContainer();
    final controller = container.read(recordingControllerProvider.notifier);
