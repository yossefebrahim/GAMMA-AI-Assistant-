...
void main() {
  ...
  test(
    'send with audio persists the clip on the user message and passes AudioInput to '
    'generate just-in-time (FR-013/FR-018)',
    () async {
      ...
      expect(
        user.audio!.path,
        contains('/${AudioFileStore.subdirectory}/'),
        reason: 'persisted via AudioFileStore at send',
      );
      expect(user.audio!.mimeType, 'audio/wav');
      expect(
        File(user.audio!.path).existsSync(),
        isTrue,
        reason: 'stored copy persisted',
      );
  ...
  test(
    'audio-only send (empty text) is allowed and titles the conversation as a voice clip '
    '(FR-004/FR-021)',
    () async {
  ...
  test(
    'an oversized clip is rejected at persist with the "record again" error and NO message '
    'row (002 DF-2 / ArgumentError)',
    () async {
      final oversized = tempClip(
        'big.wav',
        List.filled(AudioFileStore.maxBytes + 1, 0),
      );
      ...
      expect(
        read().conversationId,
        isNull,
        reason: 'send aborted before any row',
      );
      expect(gemma.lastPrompt, isNull, reason: 'generate never called');
      expect(
        container.read(recordingControllerProvider).error,
  ...
  test(
    'an unreadable temp file is rejected at persist with the same error and NO message row '
    '(002 DF-2 / FileSystemException)',
    () async {
      const missing = PendingRecording(
        path: '/nonexistent/gone.wav',
        mimeType: 'audio/wav',
        durationMs: 1500,
      );
  ...
  test(
    'a text-only send still works and passes no audio (regression)',
    () async {
