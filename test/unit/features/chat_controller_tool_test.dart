...
void main() {
  ...
  setUp(() {
    ...
    gemma = FakeGemmaService(
        capabilitiesData: const ModelCapabilities(image: true, functionCalling: true));
  ...
  test('happy path: rows read user → chip(success) → answer; empty assistant row deleted', () async {
  ...
  test('resume payload is the success result (round-trip correctness)', () async {
    gemma.scriptedEvents = [const ToolCallRequested('get_device_info', {})];
  ...
  test('text before the call: the assistant row is finalized, not deleted (ordering)', () async {
  ...
  test('handler failure → error chip + honest resume reply', () async {
    ...
    gemma.resumeEvents = [const TextDelta('sorry, the clipboard could not be read')];
  ...
  test('unknown (hallucinated) tool → Unknown error chip, handler untouched, model informed',
      () async {
  ...
  test('stop BEFORE dispatch → skipped chip, no resume, no execution (FR-026)', () async {
    ...
    expect(dispatched, isFalse, reason: 'handler must not run when stopped before dispatch');
    expect(gemma.resumeCount, 0, reason: 'no resume after a skipped tool');
  });

  test('stop AFTER dispatch but before resume → chip finalized truthfully, no resume (FR-026)',
      () async {
    handlers['get_device_info'] = (args) async {
      unawaited(controller().stop()); // user stops while the (fast, local) handler runs
      return {'batteryLevel': 83};
    };
    gemma.scriptedEvents = [const ToolCallRequested('get_device_info', {})];
    ...
    final chip = (await turns()).firstWhere((m) => m.isTool);
    expect(chip.toolStatus, ToolCallStatus.success, reason: 'handler completed truthfully');
    expect(gemma.resumeCount, 0, reason: 'no resume once stop ended the turn');
  });

  test('second tool call on resume → its own error chip, turn ends as text (FR-006/FR-024)',
      () async {
    gemma.scriptedEvents = [const ToolCallRequested('get_device_info', {})];
    // The resume itself tries to call a tool again.
    gemma.resumeEvents = [const ToolCallRequested('set_theme', {'theme': 'dark'})];
  ...
  test('attachment edge case: an image-bearing user turn → tool turn keeps the attachment + order',
      () async {
    gemma.scriptedEvents = [const ToolCallRequested('get_device_info', {})];
    gemma.resumeEvents = [const TextDelta('83%')];

    final imageFile = File('${tempDir.path}/x.jpg')..writeAsBytesSync(List.filled(64, 1));
    ...
    expect(rows.first.image, isA<ImageAttachment>(), reason: 'attachment persists unchanged');
    expect(rows.map((m) => m.role).toList(),
        [MessageRole.user, MessageRole.tool, MessageRole.assistant]);
  });
}
