...
void main() {
  ...
  test(
    'a tool turn is included as a ChatTurn.tool carrying name/args/result',
    () {
  ...
  test(
    'token accounting for a tool turn uses name + args + result JSON (oldest-drop)',
    () {
      ...
      expect(
        turns.single.isTool,
        isTrue,
        reason: 'oldest user turn dropped, tool turn kept',
      );
    },
  );

  test(
    'reserveToolInstruction shrinks the effective budget by ~40 tokens (R6)',
    () {
      ...
      final withReserve = assembler.assemble(
        messages,
