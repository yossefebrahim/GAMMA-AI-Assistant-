...
void main() {
  ...
  DownloadController controller() => container.read(downloadControllerProvider.notifier);
  ...
  test('cancel stops quickly (SC-003) and never exposes a partial as installed (FR-008/FR-011)',
      () async {
    downloader.scriptedProgress = const [
      DownloadProgress(phase: DownloadPhase.running, fraction: 0.3, downloadedBytes: 700000000),
    ];
    ...
    expect(await container.read(modelInstallRepositoryProvider).read(), isNull);
  });

  test('a partial (running-only) download is never recorded as installed (FR-011)', () async {
    downloader.scriptedProgress = const [
      DownloadProgress(phase: DownloadPhase.running, fraction: 0.2, downloadedBytes: 480000000),
      DownloadProgress(phase: DownloadPhase.running, fraction: 0.6, downloadedBytes: 1440000000),
    ];
  ...
  test('failure surfaces, then retry completes and records the install (FR-011)', () async {
    downloader.scriptedProgress = const [
      DownloadProgress(
    ...
    downloader.scriptedProgress = const [
      DownloadProgress(phase: DownloadPhase.running, fraction: 0.9, downloadedBytes: 2160000000),
      DownloadProgress(
