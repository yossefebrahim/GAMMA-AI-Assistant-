...
void main() {
  ...
  test('a prior user turn with an image yields a ChatTurn carrying its image (FR-015/FR-016)', () {
    final messages = [
      msg(1, MessageRole.user, 'what is this', image: const ImageAttachment(path: '/img/a.jpg')),
      msg(2, MessageRole.assistant, 'a cat'),
      msg(3, MessageRole.user, 'what color is it'),
    ];
    final bytes = Uint8List.fromList([1, 2, 3, 4]);

    final context = const ContextAssembler().assemble(messages, images: {1: ImageInput(bytes)});
  ...
  test('an image-only prior user turn (empty text) is still included with its image', () {
    final messages = [
      msg(1, MessageRole.user, '', image: const ImageAttachment(path: '/img/only.jpg')),
  ...
  test('on overflow the OLDEST (image) turn is dropped; stored history untouched (Q2)', () {
    const assembler = ContextAssembler(maxContextTokens: 7);
    final messages = [
      msg(1, MessageRole.user, 'a' * 20, image: const ImageAttachment(path: '/img/a.jpg')),
    ...
    final context = assembler.assemble(
      messages,
      images: {1: ImageInput(Uint8List.fromList([9]))},
    );

    // Oldest (turn 1, the image turn) dropped; the two most recent kept, in order.
    expect(context.map((t) => t.text).toList(), ['b' * 20, 'cccc']);
    expect(context.every((t) => t.image == null), isTrue, reason: 'the image turn was trimmed away');
