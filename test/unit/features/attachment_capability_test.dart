...
void main() {
  ...
  test(
    'flipping to an image-unsupported model clears the pending image with a note (FR-008)',
    () async {
  ...
  test(
    'flipping while there is no pending image is a no-op (no spurious note)',
    () async {
      expect(read().pending, isNull);
  ...
  test(
    'a load failure / reload (session not ready) keeps the image and shows no misleading note',
    () async {
      picker.libraryResult = const PickedImage(path: '/tmp/a.jpg');
      await controller().pickFromLibrary();
      expect(read().pending, isNotNull);
      ...
      expect(
        read().pending,
        isNotNull,
        reason: 'pending image retained across a transient load state',
      );
      expect(
        read().note,
        isNull,
        reason: 'no misleading capability note while the model is not loaded',
      );
    },
  );
}
