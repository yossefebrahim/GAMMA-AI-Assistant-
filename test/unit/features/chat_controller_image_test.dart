...
void main() {
  ...
  test(
    'send with an image persists it on the user message and passes ImageInput to generate '
    '(FR-012)',
    () async {
      ...
      expect(
        File(user.image!.path).existsSync(),
        isTrue,
  ...
  test(
    'the pending attachment is cleared after a successful image send (FR-004)',
    () async {
  ...
  test(
    'a text-only send still works and passes no image (regression)',
    () async {
  ...
  test('a text-only follow-up replays the earlier image to generate via history '
    ...
    await controller().send(
      'what is this',
      image: tempImage('first.jpg', [7, 7, 7, 7]),
    );
    ...
    expect(
      gemma.lastHistoryImages!.any((i) => i != null),
      isTrue,
      reason: 'a prior image turn is present in the assembled context',
    );
    expect(
      gemma.lastHistoryImages!.firstWhere((i) => i != null)!.bytes,
