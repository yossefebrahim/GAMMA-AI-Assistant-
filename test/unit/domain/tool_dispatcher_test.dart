...
void main() {
  ...
  ToolHandler recording(
    String name,
    Map<String, Object?> Function(Map<String, Object?>) body,
  ) {
    return (args) async {
  ...
  ToolDispatcher buildDispatcher({ToolHandler Function(String)? override}) {
    Map<String, ToolHandler> handlers = {
      'get_device_info': recording(
        'get_device_info',
        (_) => {'model': 'A34', 'battery': 83},
      ),
      'summarize_clipboard': recording(
        'summarize_clipboard',
        (_) => {'text': 'hello clipboard'},
      ),
      'set_theme': recording(
        'set_theme',
        (a) => {'theme': a['theme'], 'alreadyActive': false},
      ),
      'set_timer': recording(
        'set_timer',
        (a) => {'seconds': a['seconds'], 'set': true},
      ),
    };
    if (override != null) {
  ...
  test(
    'valid call → ToolSuccess; handler invoked once with the args',
    () async {
      final outcome = await buildDispatcher().dispatch('set_theme', {
        'theme': 'dark',
      });
  ...
  test('malformed args → ToolInvalidArgs; handler NEVER invoked', () async {
    final outcome = await buildDispatcher().dispatch('set_timer', {
      'seconds': 0,
    });
    expect(outcome, isA<ToolInvalidArgs>());
    expect((outcome as ToolInvalidArgs).reason, contains('seconds'));
    expect(
      invocations.containsKey('set_timer'),
      isFalse,
      reason: 'handler must not run on bad args',
    );
  });

  test(
    'unknown key → ToolInvalidArgs (strict mode), handler untouched',
    () async {
      final outcome = await buildDispatcher().dispatch('set_theme', {
        'theme': 'dark',
        'mode': 'x',
  ...
  test('handler throw → ToolFailure (dispatch NEVER throws)', () async {
    final dispatcher = buildDispatcher(
      override: (name) =>
          (args) async => throw StateError('boom in $name'),
    );
  ...
  test(
    'oversized result is truncated with truncated: true (per-tool bound)',
    () async {
  ...
  test('a result within the bound is not marked truncated', () async {
    final outcome = await buildDispatcher().dispatch(
      'get_device_info',
      const {},
    );
  ...
  group('integer coercion', () {
    test(
      'whole-valued double → coerced to int, ToolSuccess, handler receives int',
      () async {
        final outcome = await buildDispatcher().dispatch('set_timer', {
          'seconds': 300.0,
        });
    ...
    test(
      'whole-valued double below minimum → ToolInvalidArgs after coercion',
      () async {
        final outcome = await buildDispatcher().dispatch('set_timer', {
          'seconds': 0.0,
        });
