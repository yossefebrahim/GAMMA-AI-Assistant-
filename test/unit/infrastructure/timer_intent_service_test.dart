...
void main() {
  ...
  ToolDispatcher dispatcher(ProviderContainer c) =>
      c.read(toolDispatcherProvider);

  ProviderContainer build() {
    timer = FakeTimerIntentService();
    return makeContainer(
      overrides: [timerIntentServiceProvider.overrideWithValue(timer)],
    );
  }

  test(
    'valid duration → fires the intent with seconds + label; success result',
    () async {
      final c = build();
      addTearDown(c.dispose);
      final outcome = await dispatcher(
        c,
      ).dispatch('set_timer', const {'seconds': 300, 'label': 'tea'});
  ...
  test(
    'no clock app → ToolFailure("no clock app available") (FR-015)',
    () async {
      final c = build();
      addTearDown(c.dispose);
      ...
      final outcome = await dispatcher(
        c,
      ).dispatch('set_timer', const {'seconds': 60});
  ...
  test(
    'zero seconds is rejected by the schema → ToolInvalidArgs, intent never fired (US4/AS3)',
    () async {
      final c = build();
      addTearDown(c.dispose);
      final outcome = await dispatcher(
        c,
      ).dispatch('set_timer', const {'seconds': 0});

      expect(outcome, isA<ToolInvalidArgs>());
      expect(
        timer.callCount,
        0,
        reason: 'the handler must not run on an out-of-bounds duration',
      );
    },
  );
  ...
  test('over-24h is rejected by the schema (1..86400 bound)', () async {
    ...
    final outcome = await dispatcher(
      c,
    ).dispatch('set_timer', const {'seconds': 90000});
    expect(outcome, isA<ToolInvalidArgs>());
