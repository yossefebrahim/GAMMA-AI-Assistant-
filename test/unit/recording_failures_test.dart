...
void main() {
  ...
  test('RecorderUnavailableException → clear composer message, app stays usable (FR-021)',
      () async {
    ...
    await container.read(recordingControllerProvider.notifier).stopRecording();
  });

  test('backgrounding mid-recording with ≥ minimum captured keeps the clip with a note '
      '(FR-021 stop-and-keep)', () async {
    final container = makeFailureContainer();
    final controller = container.read(recordingControllerProvider.notifier);
    final clipFile = tempClip('kept.wav');
    recorder.nextStop =
        RecordedAudio(path: clipFile.path, mimeType: 'audio/wav', durationMs: 3000);
    await controller.onMicTap();

    await controller.onAppBackgrounded();

    final state = container.read(recordingControllerProvider);
    expect(state.phase, RecordingPhase.previewing, reason: '≥ min → clip kept');
  ...
  test('backgrounding mid-recording with < minimum captured discards with a note (FR-021)',
      () async {
    final container = makeFailureContainer();
    final controller = container.read(recordingControllerProvider.notifier);
    final clipFile = tempClip('short.wav');
    recorder.nextStop =
        RecordedAudio(path: clipFile.path, mimeType: 'audio/wav', durationMs: 200);
    await controller.onMicTap();
  ...
  test('backgrounding while a preview plays releases the player (Principle VIII)', () async {
    final container = makeFailureContainer();
    final controller = container.read(recordingControllerProvider.notifier);
    ...
    final state = container.read(recordingControllerProvider);
    expect(state.clip, isNotNull, reason: 'the pending clip itself survives backgrounding');
  });
}
