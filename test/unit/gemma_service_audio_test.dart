...
void main() {
  AudioInput clip([int length = 8]) =>
      AudioInput(Uint8List.fromList(List.filled(length, 1)), mimeType: 'audio/wav');
  ...
  group('FakeGemmaService models the seam gates (002 audit L6 closed)', () {
    test('guarantee 13: generate(audio:) while capabilities.audio is false throws StateError '
        'synchronously — the FFI silent-drop guard', () async {
      ...
      expect(
        ...
        throwsStateError,
        reason: 'the plugin silently drops ungated audio — the seam MUST throw instead',
      );
    });

    test('guarantee 14: both image and audio on one prompt throws StateError (spec Q3)',
        () async {
      final gemma = FakeGemmaService();
      await gemma.loadModel('/fake');
      ...
      expect(
        () => gemma.generate(
            history: const [], prompt: 'hi', image: picture(), audio: clip()),
        throwsStateError,
      );
    });
    ...
    test('history audio is recorded per turn as lastHistoryAudio (FR-017 replay assertions)',
        () async {
      final gemma = FakeGemmaService();
      await gemma.loadModel('/fake');
      gemma.scriptedDeltas = ['ok'];
      ...
      expect(gemma.lastHistoryAudio![1], isNull, reason: 'assistant turns carry no media');
    });

    test('a scripted AudioProcessingException surfaces on the stream (FR-022 path)', () async {
      final gemma = FakeGemmaService();
      await gemma.loadModel('/fake');
    ...
    test('L4 rule: a generic error on an audio-free follow-up reaches the caller UN-remapped',
        () async {
      final gemma = FakeGemmaService();
      await gemma.loadModel('/fake');
      ...
      expect(
        gemma
            ...
            .drain<void>(),
        throwsA(predicate((e) =>
            e is! AudioProcessingException && e is! ImageProcessingException)),
        reason: 'errors on turns that merely replay audio history propagate unchanged',
      );
    });
  });

  group('FlutterGemmaService pure-logic contract shape', () {
    test('guarantee 14 is state-free: both media throws StateError even before any load',
        () {
      final service = FlutterGemmaService();

      expect(
        () => service
            .generate(history: const [], prompt: 'hi', image: picture(), audio: clip())
            .first,
        throwsStateError,
      );
    });

    test('generate with no model loaded throws StateError (unchanged 001 contract)', () {
      final service = FlutterGemmaService();

      expect(
        () => service.generate(history: const [], prompt: 'hi', audio: clip()).first,
        throwsStateError,
      );
    });
  });
