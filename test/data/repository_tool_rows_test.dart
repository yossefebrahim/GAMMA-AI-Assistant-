...
void main() {
  ...
  test('append → finalize round-trips a tool row (running → success)', () async {
  ...
  test('finalize rejects the running state (terminal states only — guarantee 2)', () async {
    final conv = await repo.createConversation();
    final id = await repo.appendToolInvocation(
        conversationId: conv.id, toolName: 'set_timer', args: const {'seconds': 60});
    expect(
      () => repo.finalizeToolInvocation(id, status: ToolCallStatus.running, summary: 'x'),
      throwsArgumentError,
    );
  });

  test('appendToolInvocation rejects an empty tool name (field invariant)', () async {
    final conv = await repo.createConversation();
    expect(
      () => repo.appendToolInvocation(
          conversationId: conv.id, toolName: '  ', args: const {}),
      throwsArgumentError,
    );
  });

  test('finalize rejects a result over the 4,400-char ceiling (guarantee 4)', () async {
    final conv = await repo.createConversation();
    final id = await repo.appendToolInvocation(
        conversationId: conv.id, toolName: 'summarize_clipboard', args: const {});
    final huge = {'text': 'x' * 5000};
    expect(
      () => repo.finalizeToolInvocation(id,
          status: ToolCallStatus.success, result: huge, summary: 'clip'),
      throwsArgumentError,
    );
  });
  ...
  test('conversation delete cascades tool rows away', () async {
    final conv = await repo.createConversation();
    await repo.appendToolInvocation(
        conversationId: conv.id, toolName: 'set_theme', args: const {'theme': 'light'});
  ...
  test('sweep leaves already-finalized tool rows untouched', () async {
    final conv = await repo.createConversation();
    final id = await repo.appendToolInvocation(
        conversationId: conv.id, toolName: 'set_theme', args: const {'theme': 'dark'});
    await repo.finalizeToolInvocation(id,
        status: ToolCallStatus.success, result: const {'theme': 'dark'}, summary: 'dark theme');
    expect(await repo.sweepStaleToolInvocations(), 0);
  });
}
